...
probe=c("BrainMetalnc.22630")
probe_gene=c("ENSG00000182732")
highlight_probe=c("BrainMetalnc.22630")
window_size=500000
probe_list=probe
probe_gene_list=probe_gene
filename<-"/storage/yangjianLab/chenli/seq/PsychENCODE/metaeQTL/SMR/locos_plot/locus_input_data.BrainMetalnc.22630.txt"
finemap_file<-"/storage/yangjianLab/chenli/seq/PsychENCODE/lncRNA_eQTL/enrichment/finemap/sczfinemap/scz.rs2190864.locus.CARMA.txt"
#finemap_file<-"/storage/yangjianLab/chenli/seq/PsychENCODE/lncRNA_eQTL/enrichment/finemap/sczfinemap_ANN/scz.rs2190864.locus.CARMA.ANN.txt"
...
## bigwig
		...
		phyloP470way="/storage/yangjianLab/chenli/genome/GRCh38/hg19_ref/hg19.100way.phastCons.bw"
		phyloP470way_bw<-readBigwig(file = phyloP470way, chrom = chr,  chromstart =start,   chromend = end)
    ...
    multisignal <- plotMultiSignal(signalList, chrom = chr,
		...
		##signalList <- list(Astrocyte_bw, GABA_bw,Glutamatergic_bw, Oligodendrocyte_bw,Microglia_bw,phyloP470way_bw)
##    multisignal <- plotMultiSignal(signalList, chrom = chr,
##                               chromstart = start, chromend = end,
##                               linecolor = c(brewer.pal(n = 9,"YlGnBu")[3],
##                               							 brewer.pal(n = 9,"YlGnBu")[4],
...
##                                             brewer.pal(n = 9,"YlGnBu")[6],
##                                             brewer.pal(n = 9,"YlGnBu")[7],
##                                             brewer.pal(n = 9,"YlGnBu")[8]),
##                               label = c("Astrocyte", "GABA",
##                                         "Glutamatergic", "Oligodendrocyte","Microglia","phyloP470way"),
##                               assembly = "hg19",
##                               x = x0, y = 17.0, scale = T,
##                               width =width0, height = height0*3,
##                               default.units = "cm",
...
##      )
			    ...
			    plotGenomeLabel(
			      chrom = chr, chromstart = start, chromend = end,
			      assembly = "hg19",
			      x = x0, y = 23.5, length = width0, scale = "Mb",
			      just = c("left", "top"), default.units = "cm",
			      fontsize = 8
			    )
...
pdf("lnc-BrainMetalnc.22630.pdf",  width=10,height=13)  
gwasLocusPlot()
dev.off()			  
